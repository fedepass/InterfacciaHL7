...
exports.CdaPrfParser = void 0;
const common_1 = require("@nestjs/common");
const fast_xml_parser_1 = require("fast-xml-parser");
const uuid_1 = require("uuid");
let CdaPrfParser = class CdaPrfParser {
    constructor() {
        this.xmlParser = new fast_xml_parser_1.XMLParser({
            ...
            isArray: (name) => ['id', 'qualifier', 'translation', 'ingredient', 'entryRelationship', 'entry', 'templateId'].includes(name),
        });
    }
    parse(raw) {
        let doc;
        try {
            ...
            throw new common_1.BadRequestException(`CDA-PrF parse error: ${e.message}`);
        }
        const cd = doc.ClinicalDocument;
        if (!cd)
            throw new common_1.BadRequestException('Documento CDA non valido: elemento ClinicalDocument mancante');
        // ── PAZIENTE ──────────────────────────────────────────────────────────────
        ...
        const patIds = Array.isArray(patRole?.id) ? patRole.id : patRole?.id ? [patRole.id] : [];
        ...
        const prescribedBy = [this.txt(apNameObj?.given), this.txt(apNameObj?.family)].filter(Boolean).join(' ') || undefined;
        // ── REPARTO ───────────────────────────────────────────────────────────────
        ...
        const sections = this.getArr(structuredBody?.component);
        const prescSection = sections.find(c => this.attr(c.section?.code, 'code') === '57828-6')?.section;
        if (!prescSection)
            throw new common_1.BadRequestException('Sezione prescrizioni (LOINC 57828-6) non trovata nel documento CDA-PrF');
        const entries = this.getArr(prescSection.entry);
        const sa = entries[0]?.substanceAdministration;
        if (!sa)
            throw new common_1.BadRequestException('Nessuna substanceAdministration trovata nella sezione prescrizioni');
        // ── PRIORITÀ ──────────────────────────────────────────────────────────────
    ...
    extractDrug(sa) {
        ...
        if (ld) {
            ...
            const translations = Array.isArray(codeEl?.translation) ? codeEl.translation : codeEl?.translation ? [codeEl.translation] : [];
        ...
        if (mm) {
            ...
            const ingredients = Array.isArray(mm.ingredient) ? mm.ingredient : mm.ingredient ? [mm.ingredient] : [];
    ...
    extractPriority(sa, cd) {
        ...
        const translations = Array.isArray(cd.code?.translation) ? cd.code.translation : cd.code?.translation ? [cd.code.translation] : [];
        for (const tr of translations) {
            const qualifiers = Array.isArray(tr.qualifier) ? tr.qualifier : tr.qualifier ? [tr.qualifier] : [];
    ...
    extractFrequency(effectiveTime) {
        ...
        if (event)
            return this.attr(event, 'code') || 'N/D';
        return 'N/D';
    }
    // ── Componenti IV: solvente, volume, velocità infusione ────────────────────
    extractIVInfo(sa) {
        const rels = Array.isArray(sa.entryRelationship) ? sa.entryRelationship : sa.entryRelationship ? [sa.entryRelationship] : [];
    ...
    parseIVNote(text) {
        ...
        let solvent;
        if (/nacl|fisiologica|salina|cloruro.*sodio|0\.9%/.test(tl))
            solvent = 'NaCl 0.9%';
        else if (/glucosio|glucose|5%/.test(tl))
            solvent = 'Glucosio 5%';
        else if (/ringer/.test(tl))
            solvent = 'Ringer lattato';
    ...
    inferCategory(name, code) {
        ...
        if (/methotrex|cisplat|carboplat|cyclophos|doxorub|fluorourac|vincrist|paclitax|gemcitab|irinotecan|oxaliplat|etoposid|bleomycin/.test(n))
            return 'CHEMOTHERAPY';
        if (/cyclosporin|ciclosporin|tacrolimus|mycophenolat|micofenolat|azathioprin|sirolimus|everolimus/.test(n))
            return 'IMMUNOSUPPRESSANT';
        if (/amoxicil|ampicil|cefazolin|ceftriaxon|vancomycin|vancomicin|meropenem|piperacillin|metronidazol|ciprofloxacin|levofloxacin/.test(n))
            return 'ANTIBIOTIC';
        if (/heparin|eparina|warfarin|enoxaparin|fondaparin|dabigatran|rivaroxaban/.test(n))
            return 'ANTICOAGULANT';
        if (/glucose|glucosio|dextrose|amino.*acid|parenteral|nutriflex|kabiven/.test(n))
            return 'NUTRITION';
        if (/morphine|morfina|fentanyl|oxycodone|ossicodone|tramadol|remifentanil/.test(n))
        ...
        if (code) {
            if (code.startsWith('L01'))
                return 'CHEMOTHERAPY';
            if (code.startsWith('L04'))
                return 'IMMUNOSUPPRESSANT';
            if (code.startsWith('J01'))
                return 'ANTIBIOTIC';
            if (code.startsWith('B01'))
                return 'ANTICOAGULANT';
            if (code.startsWith('B05') || code.startsWith('V06'))
                return 'NUTRITION';
            if (code.startsWith('N02A'))
                return 'ANALGESIC_OPIOID';
...
exports.CdaPrfParser = CdaPrfParser;
exports.CdaPrfParser = CdaPrfParser = __decorate([
